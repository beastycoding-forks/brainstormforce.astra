...
describe( 'Global typography preset-6 style in the customizer', () => {
	it( 'body style should be applied correctly', async () => {
		...
		while ( false === ppStatus ) {
			await createNewPost( { postType: 'post', title: 'preset-6' } );
			await setPostContent( TPOGRAPHY_TEST_POST_CONTENT );
			ppStatus = await publishPost();
		}
		await page.goto( createURL( '/preset-6' ), {
		...
		await expect( {
			selector: 'body',
			property: 'font-size',
		} ).cssValueToBe( `${ globalTypographyPreset6[ 'font-size-body' ].desktop }${ globalTypographyPreset6[ 'font-size-body' ][ 'desktop-unit' ] }` );
		await setBrowserViewport( 'medium' );
		await expect( {
			selector: 'body',
			property: 'font-size',
		} ).cssValueToBe( `${ globalTypographyPreset6[ 'font-size-body' ].tablet }${ globalTypographyPreset6[ 'font-size-body' ][ 'tablet-unit' ] }` );
		await setBrowserViewport( 'small' );
		await expect( {
			selector: 'body',
			property: 'font-size',
		} ).cssValueToBe( `${ globalTypographyPreset6[ 'font-size-body' ].mobile }${ globalTypographyPreset6[ 'font-size-body' ][ 'mobile-unit' ] }` );
	} );
