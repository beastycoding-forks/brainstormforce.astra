...
describe( 'Add secondary menu color settings for mobile view', () => {
	it( 'secondary menu color settings should be added properly in mobile view', async () => {
		...
		await expect( {
			...
			property: 'color',
		} ).cssValueToBe( `${ secondaryMenuFontColor[ 'header-menu2-color-responsive' ].mobile }`,
		);
		...
		await expect( {
			...
			`${ secondaryMenuFontColor[ 'header-menu2-a-color-responsive' ].mobile }`,
		);
	} );
} );
