...
export async function publishPost() {
	...
	if ( isEntityPublishToggle ) {
		...
		this.click( '.editor-entities-saved-states__save-button' );
	}
	...
	if ( isPublishingPost ) {
		...
		const publishButton = this.waitForSelector(
		...
		try {
			this.waitForSelector( '.components-snackbar' );
			return true;
		} catch ( error ) {
			return false;
		}
