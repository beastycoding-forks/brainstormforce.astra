...
describe( 'Secondary menu background normal color settings in the customizer for mobile', () => {
	//test case for normal color setting
	it( 'secondary menu background normal color should apply corectly in mobile', async () => {
		...
		await expect( {
			...
			`${ secondaryMenuBackgroundColor[ 'header-menu2-color-responsive' ].mobile }`,
		);
		await expect( {
			selector: '.ast-builder-menu-2 .main-header-menu',
			property: 'background-color',
		} ).cssValueToBe(
			`${ secondaryMenuBackgroundColor[ 'header-menu2-bg-obj-responsive' ].mobile[ 'background-color' ] }`,
		);
	} );
	//test case for active color setting
	it( 'secondary menu background active color should apply corectly in mobile', async () => {
		const secondaryMenuBackgroundColor = {
		...
		await expect( {
			...
			`${ secondaryMenuBackgroundColor[ 'header-menu2-a-bg-color-responsive' ].mobile }`,
		);
	} );
} );
